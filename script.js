...
function passwordValidityCheck() {
    let passwordCheck = new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{12,})");
    if(password.value.match(passwordCheck)){
        password.setCustomValidity('');
    } else {
        password.setCustomValidity('Password must be 12 characters long, include 1 lowercase, 1 uppercase, 1 numeric character and one special character')
    }
}
